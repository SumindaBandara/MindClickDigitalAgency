  ...
  <div className="max-w-7xl mx-auto px-4 py-3 md:px-6 md:py-4 flex items-center justify-between">
    ...
    <div className="flex items-center gap-3">
      ...
      <a 
        ...
        className={`bg-gradient-to-r ${config.primaryGradient} text-white px-4 py-2 md:px-6 md:py-2.5 rounded-xl font-bold text-xs md:text-sm hover:shadow-xl hover:shadow-purple-500/50 transform hover:scale-105 transition-all`}
      >
        {config.primaryCTA}
      </a>

      {/* CHANGE 3: Hamburger Menu (Visible ONLY on Mobile 'md:hidden') */}
      <button className="md:hidden text-gray-700 hover:text-purple-600 transition-colors p-1">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
          ...
          <section className="relative -mt-12 z-20">
            {/* CHANGE 1: Adjusted horizontal padding (px-4 on mobile) */}
            <div className="max-w-6xl mx-auto px-4 md:px-6">
              
              {/* CHANGE 2: Reduced internal padding on mobile (p-6) */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
                  {config.stats.map((stat, idx) => (
                    <div key={idx} className="text-center">
                      {/* CHANGE 4: Responsive Text Size (text-3xl on mobile, 4xl on desktop) */}
                      <div className={`text-3xl md:text-4xl font-black bg-gradient-to-r ${config.primaryGradient} bg-clip-text text-transparent mb-1`}>
                        {stat.value}
                      </div>
